...
def ensure_packages_are_installed(package_names, allow_modify_external_python):
    ...
    if not use_own_python and not allow_modify_external_python:
        return handle_cannot_install_packages(package_names)
...
def install_package(name):
    ...
    subprocess.run(
        [str(python_path), "-m", "pip", "install", name, "--target", target],
        cwd=cwd_for_subprocesses,
    )

    if not module_can_be_imported(name):
        return handle_fatal_error(f"could not install {name}")
...
def get_package_install_directory() -> str:
    ...
    return handle_fatal_error(
        "Don't know where to install packages. Please make a bug report."
    )
